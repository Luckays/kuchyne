...
assert.equal(model.cleaning_access.width_m,.65);
assert.equal(model.cleaning_access.wardrobe_shape,'L');
assert(model.boxes.filter(b=>b.name.startsWith('Satna rohove kridlo')).length>5);
assert(model.boxes.filter(b=>b.group==='wc_wall').length>25);
